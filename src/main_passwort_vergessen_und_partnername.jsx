...
function ProfileEditor({
  ...
  useEffect(() => {
    ...
    async function loadWorkshopData() {
      ...
      // Namen der Tanzpartner für die Profilansicht laden.
      const partnerIds = [...new Set((pairs || []).map(pair =>
        pair.user1_id === user.id ? pair.user2_id : pair.user1_id
      ).filter(Boolean))];

      let partnerProfiles = [];
      if (partnerIds.length > 0) {
        const { data: profiles, error: partnerProfileError } = await supabase
          .from("profiles")
          .select("id,display_name")
          .in("id", partnerIds);

        if (partnerProfileError) {
          console.warn("Tanzpartner-Namen konnten nicht geladen werden:", partnerProfileError.message);
        } else {
          partnerProfiles = profiles || [];
        }
      }

      const partnerNameByUserId = Object.fromEntries(
        partnerProfiles.map(p => [p.id, p.display_name])
      );
      const partnerNameByWorkshopId = {};

      for (const pair of pairs || []) {
        const otherUserId = pair.user1_id === user.id ? pair.user2_id : pair.user1_id;
        if (otherUserId) {
          partnerNameByWorkshopId[pair.workshop_id] =
            partnerNameByUserId[otherUserId] || "Tanzpartner/in";
        }
      }
