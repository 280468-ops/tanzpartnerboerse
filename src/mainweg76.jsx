...
async function isUserBlocked(userId) {
  ...
  if (error) {
    console.error("Sperrstatus konnte nicht geprüft werden:", error);
    return false;
  }
...
function App() {
  ...
  useEffect(() => {
    ...
    async function handleSession(currentSession) {
      ...
      const blocked = await isUserBlocked(currentSession.user.id);
      ...
      if (blocked) {
        ...
        if (mounted) {
          setSession(null);
          setLoading(false);
          alert("Dein Konto wurde vom Administrator gesperrt.");
        }
        return;
      }
  ...
  async function submit(e) {
    ...
    try {
      if (mode === "register") {
        ...
        if (data?.user?.id && await isUserBlocked(data.user.id)) {
          await supabase.auth.signOut();
          throw new Error(
            "Dieses Konto wurde vom Administrator gesperrt. Eine Anmeldung ist nicht möglich."
          );
        }
      }
    } catch (err) {
...
function Workshops({ currentUser }) {
  ...
  const [myPairPartnerNames, setMyPairPartnerNames] = useState({});
  ...
  async function load() {
    ...
    // Namen des jeweils anderen Tanzpartners laden.
    const partnerIds = [
      ...new Set(
        (pairs || [])
          .map(pair =>
    ...
    const partnerNameById = Object.fromEntries(
      partnerProfiles.map(profile => [
        profile.id,
        profile.display_name || "Tanzpartner/in"
      ])
    );
    ...
    (pairs || []).forEach(pair => {
      ...
      pairNames[pair.workshop_id] =
        partnerNameById[otherId] || "Tanzpartner/in";
    });

    setMyPairPartnerNames(pairNames);
              ...
              <article className="profile-card" key={w.id}>
                ...
                {paired ? (
                  <div className="notice success">
                    ...
                    <div>
                      👤 <b>Dein Tanzpartner: {myPairPartnerNames[w.id] || "Tanzpartner/in"}</b>
                    </div>
...
function AdminPanel() {
  ...
  async function uploadBackgroundImage(file) {
    ...
    try {
      ...
      const freshUrl =
        LOGIN_BACKGROUND_URL +
        (LOGIN_BACKGROUND_URL.includes("?") ? "&" : "?") +
        "v=" +
        Date.now();

      setBackgroundPreviewUrl(freshUrl);

      alert("Das Login-Hintergrundbild wurde erfolgreich ersetzt.");
    } catch (error) {
      alert(
      ...
      {view === "menu" && (
        <div className="grid">
          ...
          <div className="profile-card">
            ...
            <p className="muted">
              Hier kannst du das Login-Hintergrundbild direkt
              als Administrator ersetzen.
            </p>
            ...
            <p className="small">
              Datei: <b>login-bg.png</b> · JPG, PNG oder WebP ·
              maximal 8 MB
            </p>
