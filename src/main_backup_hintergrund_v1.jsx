...
function AdminPanel() {
  ...
  async function replaceLoginBackground(event) {
    ...
    if (file.size > 8 * 1024 * 1024) {
      alert("Das Bild darf höchstens 8 MB groß sein.");
      return;
    }

    if (!confirm("Das aktuelle Login-Hintergrundbild wirklich ersetzen?")) {
      return;
    }
    ...
    try {
      ...
      alert("Login-Hintergrund wurde erfolgreich ersetzt. ✅");
    } catch (err) {
      alert(
        "Das Hintergrundbild konnte nicht ersetzt werden:\n\n" +
          (err.message || "Unbekannter Fehler") +
          "\n\nFalls Supabase einen Storage-/RLS-Fehler meldet, muss die Upload-Berechtigung für den Bucket app-images einmal freigegeben werden."
      );
    } finally {
  ...
  async function blockReportedUser(report) {
    ...
    if (!report.reported_user_id) {
      alert("Diese Meldung enthält keine gültige Nutzer-ID des gemeldeten Nutzers.");
      return;
    }

    const { data: savedValue, error } = await supabase.rpc("admin_set_user_blocked", {
    ...
    if (savedValue !== true) {
      alert("Die Datenbank hat die Sperre nicht bestätigt.");
      return;
    }
  ...
  async function toggleBlocked(user) {
    const newValue = !Boolean(user.is_blocked);
    const action = newValue ? "gesperrt" : "entsperrt";
    const name = user.display_name || "diesen Nutzer";

    if (!confirm(`Nutzer „${name}“ wirklich ${newValue ? "sperren" : "entsperren"}?`)) {
      return;
    }

    const { data: savedValue, error } = await supabase.rpc(
      "admin_set_user_blocked",
      {
        target_user_id: user.id,
        blocked_value: newValue
      }
    );

    if (error) {
      alert(
        `Fehler beim ${newValue ? "Sperren" : "Entsperren"}: ${error.message}` +
        "\n\nBitte pruefe, ob die neue admin_sperre_v3.txt bereits in Supabase ausgefuehrt wurde."
      );
      return;
    }

    if (savedValue !== newValue) {
      alert("Die Datenbank hat die Änderung nicht bestätigt.");
      return;
    }

    await loadUsers();
    alert(`Nutzer „${name}“ wurde ${action}.`);
  }
      ...
      {view === "menu" && (
        <div className="grid">
          ...
          <div className="profile-card">
            ...
            <label className="primary wide" style={{ display: "block", textAlign: "center", cursor: backgroundUploading ? "wait" : "pointer" }}>
              {backgroundUploading
              ...
              <input
                type="file"
                accept="image/*"
                onChange={replaceLoginBackground}
            ...
            <div className="notice">
              Das ausgewählte Bild ersetzt automatisch
              <b> login-bg.png</b> im Bucket <b>app-images</b>.
              Empfohlen: JPG oder PNG, maximal 8 MB.
            </div>
          </div>
        </div>
      )}
