...
function ProfileEditor({
  ...
  const [workshops, setWorkshops] = useState([]);
  const [workshopInterests, setWorkshopInterests] = useState({});
  ...
  useEffect(() => {
    ...
    async function loadProfileExtras() {
      ...
      if (workshopError) {
        console.error(
          "Workshops konnten im Profil nicht geladen werden:",
          workshopError
        );
      }
      ...
      if (interestError) {
        console.error(
          "Workshop-Auswahl konnte nicht geladen werden:",
          interestError
        );
      }
      ...
      (interestData || []).forEach(item => {
        interestMap[item.workshop_id] = item.level || "";
      });

      setWorkshopInterests(interestMap);
  ...
  function formatWorkshopTime(iso) {
    if (!iso) return "";
  ...
  function setWorkshopLevel(workshopId, level) {
  ...
  async function saveWorkshopInterests() {
    const selectedIds = Object.keys(workshopInterests);
    ...
    const existingIds = (existing || []).map(x => x.workshop_id);
    ...
    if (idsToDelete.length > 0) {
      const { error: deleteError } = await supabase
        ...
        .delete()
        .eq("user_id", user.id)
        .in("workshop_id", idsToDelete);

      if (deleteError) {
        throw new Error(
          "Nicht mehr ausgewählte Workshops konnten nicht entfernt werden:\n" +
            deleteError.message
        );
      }
    }
    ...
    if (selectedIds.length > 0) {
      const rows = selectedIds.map(workshopId => ({
        user_id: user.id,
        workshop_id: workshopId,
        level: workshopInterests[workshopId]
      }));
      ...
      if (upsertError) {
        throw new Error(
          "Workshop-Auswahl konnte nicht gespeichert werden:\n" +
            upsertError.message
        );
      }
    }
  ...
  async function save(e) {
    ...
    try {
      ...
      await saveWorkshopInterests();

      setForm(data);
      setProfile(data);

      alert("Profil und Workshop-Auswahl gespeichert ✅");
    } catch (err) {
      alert(
        err.message ||
  ...
  return (
    <section>
      ...
      >
        ...
        <h3>🎟️ Meine Workshops</h3>

        <p className="muted">
          Wähle hier die Workshops aus, für die du
          einen Tanzpartner suchst. Das gewählte
          Tanzniveau wird direkt beim jeweiligen
        ...
        <label className="check">
          ...
          📧 Über neue Workshops per E-Mail informiert
          werden
        </label>
