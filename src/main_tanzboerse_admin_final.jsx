...
function AdminPanel() {
  ...
  useEffect(() => {
    ...
    if (view === "contacts") loadContactRequests();
  ...
  async function toggleBlocked(user) {
    ...
    setUsers(prev =>
      prev.map(u =>
        u.id === user.id ? { ...u, is_blocked: newValue } : u
      )
    );
  }

  async function callAdminDeleteUser(userId) {
    ...
    if (!sessionData?.session?.access_token) {
      throw new Error("Keine aktive Anmeldung.");
    }
  ...
  async function deleteUser(user) {
    const confirmed = confirm(
      `Nutzer "${user.display_name || "Ohne Namen"}" wirklich vollständig löschen?\n\n` +
      "Das Login-Konto und das Profil werden gelöscht. " +
    ...
    try {
      await callAdminDeleteUser(user.id);

      setUsers(prev => prev.filter(u => u.id !== user.id));

      alert(
        "Der Nutzer wurde vollständig gelöscht.\n\n" +
        "Das Login-Konto und das Profil wurden entfernt."
      );
    } catch (err) {
      console.error("Admin-Löschen fehlgeschlagen:", err);

      alert(
        "Nutzer konnte nicht vollständig gelöscht werden:\n\n" +
        (err.message || "Unbekannter Fehler") +
        "\n\nBitte prüfe, ob die Edge Function 'admin-delete-user' in Supabase deployed wurde."
      );
    }
  }
  ...
  async function loadReports() {
    ...
    if (error) {
      alert(
        ...
        error.message +
        "\n\nFalls die Relationsabfrage nicht unterstützt wird, prüfen wir die Foreign Keys in Supabase."
      );
      return;
    }
  ...
  async function closeReport(report) {
    ...
    if (error) {
      alert("Meldung konnte nicht geschlossen werden: " + error.message);
      return;
    }
  ...
  async function blockReportedUser(report) {
    ...
    const name =
      report.reported?.display_name || "gemeldeter Nutzer";

    if (
      !confirm(
        `"${name}" wirklich sperren?\n\nDer Nutzer bleibt registriert, kann aber nicht mehr als sichtbarer Tanzpartner erscheinen.`
      )
    ) {
      return;
    }
    ...
    if (error) {
      alert("Nutzer konnte nicht gesperrt werden: " + error.message);
      return;
    }
    ...
    await loadReports();
    alert("Nutzer wurde gesperrt und die Meldung geschlossen.");
  }

  async function deleteReportedUser(report) {
    const userId = report.reported_user_id;
    const name = report.reported?.display_name || "gemeldeter Nutzer";
    ...
    if (
      !confirm(
        ...
        "Das Profil wird gelöscht. Für die vollständige Löschung des Login-Kontos benötigen wir die sichere Supabase-Edge-Function."
      )
    ) {
      return;
    }
    ...
    await loadReports();
    await loadUsers();
    alert("Nutzer wurde vollständig gelöscht und die Meldung geschlossen.");
  }
  ...
  async function loadParticipants(workshopId) {
    ...
    if (pairError) {
      alert("Fehler beim Laden der Partnerschaften: " + pairError.message);
      return;
    }

    const pairIds = (pairData || []).flatMap(p => [
      p.user1_id,
      p.user2_id
    ]);
    ...
    if (pairIds.length) {
      const { data: pp, error: ppError } = await supabase
        .from("profiles")
        .select("id,display_name")
        .in("id", [...new Set(pairIds)]);

      if (ppError) {
        alert("Fehler beim Laden der Partnernamen: " + ppError.message);
        return;
      }
    ...
    const pairMap = (pairData || []).map(pair => ({
      ...pair,
      user1: pairProfiles.find(p => p.id === pair.user1_id),
      user2: pairProfiles.find(p => p.id === pair.user2_id)
    }));
  ...
  async function editWorkshop(workshop) {
    ...
    const bookingUrl =
      prompt("Eventfrog-Link (optional):", workshop.booking_url || "") || "";
  ...
  async function deleteWorkshop(workshop) {
    if (!confirm(`Workshop "${workshop.title}" wirklich löschen?`)) return;
  ...
  async function loadContactRequests() {
    ...
    if (error) {
      alert("Fehler beim Laden der Kontaktanfragen: " + error.message);
      return;
    }
  ...
  return (
    <section>
      ...
      {view === "menu" && (
        <div className="grid">
          ...
          >
            ...
            <p className="muted">
              Nutzer anzeigen, sperren, entsperren und löschen.
            </p>
          </button>
          ...
          >
            ...
            <p className="muted">
              Meldungen prüfen, schließen sowie Nutzer sperren oder löschen.
            </p>
          </button>
          ...
          >
            <h3>💃 Workshops & Tanzpartner</h3>
            <p className="muted">
              Teilnehmer, Suchende und bestehende Partnerschaften anzeigen.
            </p>
          </button>
          ...
          >
            ...
            <p className="muted">Kontaktanfragen verwalten.</p>
          </button>

          <div className="profile-card">
            <h3>🎨 Login-Hintergrund</h3>
            <p className="muted">
              Das Bild auf der Anmeldeseite kann hier direkt ausgetauscht werden.
            </p>
            ...
            <p className="small">JPG oder PNG · maximal 8 MB</p>
          </div>
        </div>
      )}
      ...
      {view === "users" && (
        <div>
          <button onClick={() => setView("menu")}>← Zurück</button>
          ...
          {users.map(user => (
            <article className="profile-card" key={user.id}>
              ...
              <p>
                Status: {user.is_blocked ? "🔒 Gesperrt" : "🟢 Aktiv"}
              </p>
              ...
              >
                <button onClick={() => toggleBlocked(user)}>
      ...
      {view === "reports" && (
        <div>
          <button onClick={() => setView("menu")}>← Zurück</button>
          <h2>🚨 Nutzermeldungen</h2>

          {reportsLoading && <p>Meldungen werden geladen …</p>}

          {!reportsLoading && reports.length === 0 && (
            <p className="muted">Keine Meldungen vorhanden.</p>
          )}

          {!reportsLoading &&
            reports.map(report => (
              <article className="profile-card" key={report.id}>
                <h3>
                ...
                <p className="muted">
                  Meldung am {formatDate(report.created_at)} um{" "}
                  {formatTime(report.created_at)} Uhr
                </p>
                ...
                {report.status === "pending" && (
                  ...
                  >
                    ...
                    <button onClick={() => blockReportedUser(report)}>
                      🔒 Nutzer sperren
                    </button>

                    <button
                      className="ghost"
                      onClick={() => deleteReportedUser(report)}
                      style={{ color: "#b00020" }}
                    >
                      🗑️ Nutzer löschen
                    </button>
      ...
      {view === "workshops" && (
        <div>
          <button onClick={() => setView("menu")}>← Zurück</button>
          <h2>💃 Workshop- & Tanzpartner-Verwaltung</h2>
          ...
          {workshops.map(workshop => (
            <article className="profile-card" key={workshop.id}>
              <h3>
              ...
              {workshop.location && <p>📍 {workshop.location}</p>}

              <button onClick={() => loadParticipants(workshop.id)}>
                👥 Teilnehmer & Partnerschaften anzeigen
              </button>
              ...
              {participants[workshop.id] && (
                <div style={{ marginTop: "15px" }}>
                  ...
                  {participants[workshop.id].length === 0 && (
                    <p className="muted">Noch keine Suchenden.</p>
                  )}
                  ...
                  {participants[workshop.id].map(participant => (
                    ...
                    >
                      ...
                      <div>
                        Niveau:{" "}
                        {participant.level || "nicht angegeben"}
                      </div>
                    </div>
                  ))}

                  <h3 style={{ marginTop: "18px" }}>
                    ❤️ Bestehende Partnerschaften
                  </h3>

                  {(pairs[workshop.id] || []).length === 0 ? (
                    <p className="muted">
                      Für diesen Workshop wurde noch keine Partnerschaft
                      gespeichert.
                    </p>
                  ) : (
                    (pairs[workshop.id] || []).map((pair, index) => (
                      <div
                        key={`${pair.user1_id}-${pair.user2_id}-${index}`}
                        style={{
                          padding: "10px 0",
                          borderBottom: "1px solid #eee"
                        }}
                      >
                        👤{" "}
                        <strong>
                          {pair.user1?.display_name || "Unbekannt"}
                        </strong>
                        {" ❤️ "}
                        <strong>
                          {pair.user2?.display_name || "Unbekannt"}
                        </strong>
                      </div>
                    ))
                  )}
      ...
      {view === "contacts" && (
        <div>
          ...
          <article className="profile-card">
            ...
            <p className="muted">
              Hier siehst du alle Kontaktanfragen zwischen den Nutzern.
          ...
          {!contactLoading &&
            contactRequests.map(request => {
              ...
              return (
                <article className="profile-card" key={request.id}>
                  <h3>
                  ...
                  {workshop && (
                    <p>
                      🎟️ <strong>Workshop:</strong> {workshop.title}
                      {workshop.starts_at
                        ? ` · ${formatDate(workshop.starts_at)} ${formatTime(
                            workshop.starts_at
                          )} Uhr`
                        : ""}
                    </p>
                  ...
                  <p className="muted">
                    Anfrage am {formatDate(request.created_at)} um{" "}
                    {formatTime(request.created_at)} Uhr
                  </p>
                </article>
              );
            })}
