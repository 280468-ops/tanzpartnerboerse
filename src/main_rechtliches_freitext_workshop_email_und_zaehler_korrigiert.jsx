...
function AdminPanel() {
  ...
  async function addWorkshop() {
    ...
    // Workshop wurde erfolgreich gespeichert. Jetzt werden alle
    // Nutzer informiert, die E-Mail-Benachrichtigungen für neue
    // Workshops aktiviert haben. Blockierte Nutzer werden ausgeschlossen.
    const { data: notificationProfiles, error: notificationProfilesError } =
      await supabase
        .from("profiles")
        .select("id,display_name")
        .eq("is_blocked", false)
        .or(
          "workshop_email_notifications.eq.true,workshop_email_notifications.is.null"
        );

    if (notificationProfilesError) {
      console.warn(
        "Workshop wurde veröffentlicht, aber die Empfänger für die E-Mail konnten nicht geladen werden:",
        notificationProfilesError.message
      );
    } else {
      ...
      for (const recipient of notificationProfiles || []) {
        try {
          const emailResult = await sendPartnerEmail({
            recipientId: recipient.id,
            recipientName:
              recipient.display_name || "Tanzfreund/in",
            partnerName: "Peter & Bettina's Tanzpartnerbörse",
          ...
          if (!emailResult.ok) {
            console.warn(
              "Workshop-E-Mail konnte nicht an " +
          ...
          console.warn(
            "Fehler beim Versand der Workshop-E-Mail:",
            emailError
          );
        }
      }
