          ...
          <div>
            {myWorkshops.slice(0, 4).map(w => (
              <div key={w.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 22px", borderTop: "1px solid #eee" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div className="small" style={{ fontSize: 14 }}>{formatWorkshopDate(w.starts_at)} · Sonntag</div>
                  <strong style={{ fontSize: 18, lineHeight: 1.25, fontWeight: 700 }}>{w.title}</strong>
                </div>
                <span style={{ color: w.paired ? "#39824d" : "#6f35d9", fontSize: 12, fontWeight: 700 }}>{w.paired ? "Partner" : "Suche"}</span>
              </div>
            ))}
          </div>
        )}
