  ...
  return (
    <section>
      ...
      {items.length ? (
        <div className="grid">
          {items.map(x => (
            <article className="profile-card" key={x.favorite_user_id}>
              <div className="avatar">💃</div>

              <h3>
                {x.profiles?.display_name}
                {x.profiles?.age ? `, ${x.profiles.age}` : ""}
              </h3>
            </article>
          ))}
        </div>
      ) : (
        <div className="card">Noch keine Favoriten.</div>
      )}
    </section>
  );
}
...
function Chat({ userId, contact, onBack }) {
  ...
  async function loadMessages() {
    let q = supabase
      .from("messages")
      .select(
        "id,sender_id,recipient_id,workshop_id,body,created_at,is_read"
      )
      .or(
              ...
              if (m.recipient_id === userId && m.is_read === false) {
                supabase
                  ...
                  .then(({ error: readError }) => {
                    if (readError) {
                      console.error(
                        "Neue Nachricht konnte nicht als gelesen markiert werden:",
                        readError
                      );
                    }
                  });
