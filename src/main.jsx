...
function ProfileEditor({
  ...
  async function save(e) {
    ...
    try {
      ...
      // Wichtig: Immer die tatsächlich eingeloggte Supabase-User-ID
      // verwenden. Damit stimmt profiles.id garantiert mit auth.uid()
      // überein und die RLS-Regeln für das eigene Profil greifen.
      const {
        data: authData,
        error: authError
      ...
      if (authError || !authData?.user) {
        throw new Error(
          "Die Anmeldung ist abgelaufen. Bitte einmal abmelden und wieder anmelden."
        );
      }

      const currentUser = authData.user;

      // Die ID darf niemals aus einem Formular kommen.
      payload.id = currentUser.id;

      // Erst versuchen wir ein UPDATE des eigenen Profils.
      // Falls es noch nicht existiert, legen wir es mit INSERT an.
      const {
        data: updatedProfile,
        error: updateError
      } = await supabase
      ...
      // Bei einem neuen Benutzer existiert eventuell noch kein Profil.
      if (!data) {
        const {
