...
function ProfileEditor({
  ...
  useEffect(() => {
    ...
    async function loadWorkshopData() {
      ...
      // Für jeden akzeptierten Workshop-Paar-Eintrag den tatsächlichen
      // Tanzpartner des aktuellen Benutzers ermitteln.
      const partnerIds = [
        ...new Set(
          (pairs || [])
            .map(pair =>
      ...
      if (partnerIds.length > 0) {
        const { data: profiles, error: partnerError } = await supabase
          .from("profiles")
          .select("id,display_name")
          .in("id", partnerIds);
      ...
      for (const pair of pairs || []) {
        ...
        } else if (pair.user2_id === user.id) {
          partnerId = pair.user1_id;
        }

        if (partnerId) {
          partnerByWorkshop[pair.workshop_id] =
            partnerNameMap[partnerId] || "Tanzpartner/in";
        }
      }
      ...
      const list = workshops.map(w => ({
        ...
        partnerName: partnerByWorkshop[w.id] || null
      }));
            ...
            {myWorkshops.slice(0, 4).map(w => (
              <div key={w.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 22px", borderTop: "1px solid #eee" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {w.paired ? (
                    <div style={{ color: "#39824d", fontSize: 14, fontWeight: 700, marginTop: 3 }}>
                      🤝 Tanzpartner: {w.partnerName || "Tanzpartner/in"}
                    </div>
                  ) : (
                    <div style={{ color: "#6f35d9", fontSize: 14, fontWeight: 700, marginTop: 3 }}>
                      🔎 Noch keinen Tanzpartner
                    </div>
                  )}
                </div>
              </div>
...
function Contacts({ userId }) {
  ...
  async function load() {
    ...
    // damit in den Kontakten sofort erkennbar ist, um welchen
    // Workshop es geht.
    const workshopIds = [
      ...new Set(
        (requests || [])
      ...
      <div className="card row" key={item.id}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {item.workshop_id && workshopDetails[item.workshop_id] && (
            ...
            >
              💃 {workshopDetails[item.workshop_id].title}
