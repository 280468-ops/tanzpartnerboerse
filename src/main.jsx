                    ...
                    <>
                      ...
                      {myInterests.has(openWorkshop.id) && (
                        <button
                          type="button"
                          className="ghost"
                          onClick={() => withdrawPartnerSearch(openWorkshop.id)}
                          style={{
                            width: "100%",
                            borderColor: "#e6b8b8",
                            color: "#b42318",
...
function ProfileEditor({
  ...
  async function withdrawPartnerSearch(workshopId) {
    ...
    if (error) {
      ...
      alert("Die Tanzpartnersuche konnte nicht zurückgezogen werden:\n" + error.message);
      return;
    }

    // Die Suche ist damit nicht mehr offen. Der Workshop-Eintrag wird
    // ebenfalls aus "Meine Workshops" entfernt, weil die aktuelle
    // Datenstruktur die Workshop-Anmeldung und Partnersuche gemeinsam
    // in workshop_interests speichert.
    setMySearches(prev => prev.filter(w => w.id !== workshopId));
    setMyWorkshops(prev => prev.filter(w => w.id !== workshopId));

    alert("Tanzpartnersuche wurde zurückgezogen.");
  }
