          ...
          >
            <img
              ...
              onError={e => {
                ...
                const ext = current.dataset.ext || "";
                const next = ext === "" ? "jpg" : ext === "jpg" ? "jpeg" : ext === "jpeg" ? "webp" : "fallback";
                if (next !== "fallback" && workshop?.id != null && !workshop?.image_url) {
                  current.dataset.ext = next;
                  current.src = `${WORKSHOP_STORAGE_BASE_URL}/${workshop.id}.${next}`;
                  return;
                }
                  ...
                  <div className="workshop-main-row">
                    <img
                      ...
                      onError={e => {
                        ...
                        const ext = current.dataset.ext || "";
                        const next = ext === "" ? "jpg" : ext === "jpg" ? "jpeg" : ext === "jpeg" ? "webp" : "fallback";
                        if (next !== "fallback" && w?.id != null && !w?.image_url) {
                          current.dataset.ext = next;
                          current.src = `${WORKSHOP_STORAGE_BASE_URL}/${w.id}.${next}`;
                          return;
                        }
                        current.onerror = null;
                        current.src = WORKSHOP_HEADER_URL;
                ...
                <div className="workshop-modal-header">
                  <img
                    ...
                    onError={e => {
                      ...
                      const ext = current.dataset.ext || "";
                      const next = ext === "" ? "jpg" : ext === "jpg" ? "jpeg" : ext === "jpeg" ? "webp" : "fallback";
                      if (next !== "fallback" && openWorkshop?.id != null && !openWorkshop?.image_url) {
                        current.dataset.ext = next;
                        current.src = `${WORKSHOP_STORAGE_BASE_URL}/${openWorkshop.id}.${next}`;
                        return;
                      }
                      current.onerror = null;
                      current.src = WORKSHOP_HEADER_URL;
...
function AdminPanel({ onBack = null, demoMode = false }) {
  ...
  async function deleteWorkshopHeader() {
    ...
    try {
      ...
      let { error } = await adminDb.storage.from("app-images").remove(["workshop-header.png"]);
      if (error) {
        ...
        const response = await fetch(endpoint, {
          ...
          body: JSON.stringify({ prefixes: ["workshop-header.png"] })
        });
        if (!response.ok) {
          let detail = "";
          try { detail = await response.text(); } catch (_) {}
