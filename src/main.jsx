...
function AdminPanel() {
  ...
  async function toggleBlocked(user) {
    ...
    const ok = window.confirm(
      newValue
        ? "Diesen Nutzer wirklich sperren?"
        : "Diesen Nutzer wirklich entsperren?"
    );
    ...
    try {
      ...
      if (!data?.success) {
        throw new Error(
          data?.error || "Änderung konnte nicht durchgeführt werden."
        );
      }
      ...
      alert(
        "Fehler beim Ändern des Nutzerstatus: " +
          (err.message || "Unbekannter Fehler")
      );
      setLoading(false);
    }
  ...
  async function loadReports() {
    ...
    try {
      const { data, error } =
        await supabase.rpc("admin_list_reports");
  ...
  async function closeReport(reportId) {
    if (!window.confirm("Diese Meldung wirklich schließen?")) {
      return;
    }
    ...
    try {
      const { data, error } =
        await supabase.rpc("admin_close_report", {
      ...
      if (!data?.success) {
        throw new Error(
          data?.error || "Meldung konnte nicht geschlossen werden."
        );
      }
      ...
      alert(
        "Fehler beim Schließen der Meldung: " +
          (err.message || "Unbekannter Fehler")
      );
      setLoading(false);
    }
  ...
  async function blockReportedUser(reportId) {
    if (
      !window.confirm(
        "Gemeldeten Nutzer sperren und diese Meldung schließen?"
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (!data?.success) {
        throw new Error(
          data?.error || "Nutzer konnte nicht gesperrt werden."
        );
      }
      ...
      alert(
        "Fehler beim Sperren des Nutzers: " +
          (err.message || "Unbekannter Fehler")
      );
      setLoading(false);
    }
  ...
  async function loadAdminWorkshops() {
    ...
    try {
      const { data, error } =
        await supabase.rpc("admin_list_workshops");
      ...
      alert(
        "Fehler beim Laden der Admin-Workshopdaten: " +
          (err.message || "Unbekannter Fehler")
      );
    } finally {
      setLoading(false);
      ...
      {view === "menu" && (
        <div className="grid">
          ...
          >
            ...
            <p className="muted">
              Meldungen ansehen, schließen und gemeldete Nutzer sperren.
            </p>
          </button>
      ...
      {view === "reports" && (
        <div>
          ...
          {reports.map(report => (
            ...
            >
              <h3>
                🚨 {report.reported_name || "Unbekannter Nutzer"}
              </h3>

              <p>
                <b>Gemeldet von:</b>{" "}
                {report.reporter_name || "Unbekannt"}
              </p>
